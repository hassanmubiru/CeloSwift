...
class ThirdwebWalletService extends EventEmitter {
  private static instance: ThirdwebWalletService;
  ...
  private readonly STORAGE_KEYS = {
    CONNECTION_DATA: 'thirdweb_wallet_connection',
  };
  ...
  public static getInstance(): ThirdwebWalletService {
    if (!ThirdwebWalletService.instance) {
      ThirdwebWalletService.instance = new ThirdwebWalletService();
    }
    return ThirdwebWalletService.instance;
  }

  // Initialize Thirdweb SDK
  async initialize(): Promise<boolean> {
    try {
      console.log('ThirdwebWalletService: Initializing Thirdweb SDK...');
      ...
      console.log('ThirdwebWalletService: Thirdweb SDK initialized successfully');
      return true;
    } catch (error) {
      console.error('ThirdwebWalletService: Initialization failed:', error);
      return false;
    }
  }
  ...
  async connect(): Promise<boolean> {
    try {
      console.log('ThirdwebWalletService: Starting wallet connection...');
      ...
      console.log('ThirdwebWalletService: Wallet connected successfully:', address);
      ...
      console.error('ThirdwebWalletService: Connection error:', error);
      ...
      if (error.message.includes('User rejected')) {
        ...
        errorMessage = 'Please switch to Celo Alfajores network in MetaMask.';
      }
  ...
  async disconnect(): Promise<void> {
    try {
      ...
      console.log('ThirdwebWalletService: Disconnected successfully');
    } catch (error) {
      console.error('ThirdwebWalletService: Disconnect error:', error);
    }
  }
  ...
  async updateBalance(): Promise<void> {
    try {
      ...
      console.error('ThirdwebWalletService: Update balance error:', error);
    }
  }
  ...
  async switchToCeloNetwork(): Promise<boolean> {
    try {
      ...
      console.log('ThirdwebWalletService: Switched to Celo Alfajores network');
      return true;
    } catch (error) {
      console.error('ThirdwebWalletService: Network switch error:', error);
      return false;
    }
  }
  ...
  async sendTransaction(to: string, amount: string): Promise<string> {
    try {
      ...
      console.log('ThirdwebWalletService: Transaction sent:', receipt.hash);
      return receipt.hash;
    } catch (error) {
      console.error('ThirdwebWalletService: Send transaction error:', error);
      throw error;
    }
  }
  ...
  private generateSessionId(): string {
    return 'thirdweb_' + Date.now() + '_' + Math.random().toString(36).substr(2, 9);
  }

  // Handle errors
  private handleError(message: string, error: any): void {
    console.error(`ThirdwebWalletService: ${message}:`, error);
  ...
  private async saveConnectionData(): Promise<void> {
    try {
      ...
      console.error('ThirdwebWalletService: Save connection data error:', error);
    }
  }
  ...
  private async loadConnectionData(): Promise<void> {
    try {
      ...
      console.error('ThirdwebWalletService: Load connection data error:', error);
    }
  }
  ...
  private async clearConnectionData(): Promise<void> {
    try {
      ...
      console.error('ThirdwebWalletService: Clear connection data error:', error);
    }
  }
}

export default ThirdwebWalletService.getInstance();
